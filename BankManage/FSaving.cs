...
namespace BankManage
{
    public partial class FSaving : Form
    {
        ...
        {
            bool isMonthBorrow = cbTerm.Text.Contains("12 thang voi lai suat 8%/ nam");
            if (isMonthBorrow)
                dtpEnd.Value = dtpSend.Value.AddYears(1);
            else
                dtpEnd.Value = dtpSend.Value.AddYears(3);
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            TimeSpan Time = DateTime.Now.Date - dtpSend.Value.Date;           
            int TongSoNgay = Time.Days;
            if (cbTerm.Text.Contains("12 thang voi lai suat 8%/ nam"))
            {
                laisuat = 8;
            }
            else
            ...
            if (dtpEnd.Value.Date == DateTime.Now.Date)
            {
                ...
                {
                    currentCustomer.Money = currentCustomer.Money + Convert.ToInt32(loans);
                    customerDAO.UpdateMoney(currentCustomer);
                    string randomMaSo = RandomMaSo();
                    Saving newSaving = new Saving()
                    {
                ...
                {
                    string randomMaSo = RandomMaSo();
                    int txtMoneysend_update = Convert.ToInt32(txtMoneySend.Text) + Convert.ToInt32(loans);
                    Saving newSaving = new Saving()
                    {
